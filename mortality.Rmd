...
include_graphics("figures/national_mortality_rates.pdf")
```
...
ggsave(filename = "figures/excess-by-cause.pdf", out, height = 4.25, width = 7)
ggsave(filename = "figures/excess-by-cause.png", out, height = 4.25, width = 7)

```

```{r, echo = FALSE, fig.align = "center", fig.show="asis", out.width="90%"}
include_graphics("figures/excess-by-cause.pdf")
```
...
--- North Carolina reporting is very delayed.
...
out <- df_state_excess %>% 
  ...
  ggplot(aes(x = pct_excess/100, 
             y = reorder(jurisdiction, pct_excess_2020), 
...
out <- out + theme(strip.text = element_text(size = rel(1.2), hjust = 0), 
          ...
          axis.text.y = element_text(face = c(rep("plain", 4), "bold", rep("plain", 7))))


ggsave(filename = "figures/excess_by_cause_jurisdiction.pdf", out, height = 12, width = 8)
ggsave(filename = "figures/excess_by_cause_jurisdiction.png", out, height = 12, width = 8, dpi = 200)

```

```{r, echo = FALSE, fig.align = "center", fig.show="asis"}
include_graphics("figures/excess_by_cause_jurisdiction.pdf")
```
...
out <- df_state_excess %>% 
   filter(cause %in% c("All Cause"), 
          jurisdiction %nin% "North Carolina") %>% 
  mutate(yr_ind = ifelse(year == 2020, TRUE, FALSE)) %>%
  ggplot(aes(x = pct_excess/100, 
  ...
  labs(x = "  Percent above or below the average number of deaths, 2015-2019", 
       ...
       subtitle = " Provisional data. Blue bars show +/- 2 standard deviations around the 2015-2019 mean.\n Jurisdictions are ordered from highest to lowest percentage difference from the 2015-19 baseline.",
       caption = paste0("North Carolina is omitted due to reporting delays. Data: CDC. Calculations and Graph: Kieran Healy / @kjhealy\nThis graph was made on ", timestamp)) 

out <- out + theme(legend.justification = "left", 
          panel.border  = element_rect(color = "gray90", size = rel(0.9), linetype="solid", fill = NA),
          axis.text.y = element_text(face = c(rep("plain", 29), "bold", rep("plain", 24))), 
...
ggsave(filename = "figures/excess_all_cause_by_jurisdiction.png", out, height = 12, width = 8, dpi = 200)


```

```{r, echo = FALSE, fig.align = "center", fig.show="asis", out.width="90%"}
include_graphics("figures/excess_all_cause_by_jurisdiction.pdf")
```
